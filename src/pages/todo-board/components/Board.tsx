...
import { useBoardStoreActions, useGetBoardState } from "../board-store";
...
import { PLACEHOLDER_ID } from "./common-types";
...
export function Board() {
  ...
  const [items, setItems] = useState<Items>(() => {
    return Object.fromEntries(
      Object.entries(getBoardState().lanes).map(([id, lane]) => [
        id,
        lane.cards,
      ]),
    );
  ...
  const [containers, setContainers] = useState(() => {
    return getBoardState().laneOrder;
  });
  ...
  const handleAddCard = useCallback(
    (containerId: UniqueIdentifier) => {
      const newCard = addCard(containerId, {
        title: "",
        description: "",
      });
